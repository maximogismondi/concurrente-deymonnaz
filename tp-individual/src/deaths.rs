pub struct Death {
    ...
    killer_position_x: f64,
    killer_position_y: f64,
    // map: String,
    // match_id: String,
    // time: usize,
    // victim_name: String,
    // victim_placement: f64,
    victim_position_x: f64,
    victim_position_y: f64,
}
...
impl Death {
    pub fn from_csv_record(record: String) -> Result<Self, String> {
        ...
        let killer_position_x = fields[3].parse::<f64>().map_err(|e| e.to_string())?;
        let killer_position_y = fields[4].parse::<f64>().map_err(|e| e.to_string())?;
        ...
        let victim_position_x = fields[10].parse::<f64>().map_err(|e| e.to_string())?;
        let victim_position_y = fields[11].parse::<f64>().map_err(|e| e.to_string())?;
